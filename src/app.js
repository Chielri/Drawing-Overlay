...
const DOM = {
  ...
  // Calibration
  calibStatus: $('calib-status'), calibUnit: $('calib-unit'),
};
...
// Drawing state
let drawTool = 'pan'; // 'pan' | 'pen' | 'line' | 'arrow' | 'rect' | 'text' | 'measure' | 'dimension'
let drawStrokes = {}; // per-page arrays of stroke objects (single layer)
let _drawCurrent = null; // in-progress stroke

// Calibration state: pixels per real-world unit
let calibrationPixelsPerUnit = 0; // 0 = uncalibrated
let calibrationUnit = 'mm'; // 'mm' | 'cm' | 'm' | 'ft' | 'in'

// Measure tool: last measurement result shown as on-canvas overlay
let _measureResult = null; // { x1, y1, x2, y2, distPx, distReal, unit } or null
...
function applyDefaults(d) {
  ...
  if (d.calibrationPixelsPerUnit != null) calibrationPixelsPerUnit = d.calibrationPixelsPerUnit;
  if (d.calibrationUnit) { calibrationUnit = d.calibrationUnit; DOM.calibUnit.value = calibrationUnit; }
...
function gatherUISettings() {
  return {
    ...
    calibrationPixelsPerUnit, calibrationUnit
  };
}
...
window.addEventListener('keydown',e=>{
  ...
  if(e.key==='m'&&!e.ctrlKey&&!e.metaKey){setDrawTool('measure');return;}
  if(e.key==='d'&&!e.ctrlKey&&!e.metaKey){setDrawTool('dimension');return;}
...
function formatMeasurement(distPx) {
  if (calibrationPixelsPerUnit > 0) {
    const real = distPx / calibrationPixelsPerUnit;
    return real < 0.01 ? real.toExponential(2) + ' ' + calibrationUnit
         : real < 10 ? real.toFixed(2) + ' ' + calibrationUnit
         : real < 1000 ? real.toFixed(1) + ' ' + calibrationUnit
         : real.toFixed(0) + ' ' + calibrationUnit;
  }
  return distPx.toFixed(1) + ' px';
}

function startCalibration() {
  // Enter measure-like mode: user draws a line, then enters the known distance
  setDrawTool('measure');
  _calibratingMode = true;
  updateCalibrationStatus();
}

let _calibratingMode = false;

function finishCalibration(distPx) {
  const knownStr = prompt('Enter the known real-world distance for this segment (e.g. "500"):');
  if (!knownStr) { _calibratingMode = false; updateCalibrationStatus(); return; }
  const known = parseFloat(knownStr);
  if (isNaN(known) || known <= 0) { alert('Invalid distance.'); _calibratingMode = false; updateCalibrationStatus(); return; }
  calibrationUnit = DOM.calibUnit.value;
  calibrationPixelsPerUnit = distPx / known;
  _calibratingMode = false;
  _measureResult = null;
  updateCalibrationStatus();
  // Re-render draw layers to update any dimension labels
...
function updateCalibrationStatus() {
  const el = DOM.calibStatus;
  if (!el) return;
  if (_calibratingMode) {
    el.textContent = 'Draw a line of known length...';
    el.className = 'align3-status active';
  } else if (calibrationPixelsPerUnit > 0) {
    const ratio = (1 / calibrationPixelsPerUnit).toFixed(4);
    el.textContent = `1 px = ${ratio} ${calibrationUnit}`;
    el.className = 'align3-status done';
  } else {
    el.textContent = 'Not calibrated (showing px)';
    el.className = 'align3-status';
  }
}
...
// Cursor map per tool
const _toolCursors = {
  pan: 'grab', pen: 'crosshair', line: 'crosshair', arrow: 'crosshair', measure: 'crosshair', dimension: 'crosshair',
...
function setDrawTool(tool) {
  drawTool = tool;
  // Clear measure overlay when switching away from measure
  if (tool !== 'measure' && _measureResult) {
    _measureResult = null;
    if (mode === 'overlay') renderDrawLayer('overlay');
    else { renderDrawLayer('old'); renderDrawLayer('new'); }
  }
  ['pan','pen','line','arrow','highlight','rect','text','eraser','measure','dimension'].forEach(t => {
...
function renderDrawLayer(side) {
  ...
  // Draw measure result overlay
  if (_measureResult) {
    drawMeasureOverlay(ctx, _measureResult);
  }
}

function drawMeasureOverlay(ctx, m) {
  ctx.save();
  ...
  // Label at midpoint
  const label = formatMeasurement(dist);
  const fontSize = 16;
  ctx.font = `600 ${fontSize}px 'JetBrains Mono', monospace`;
  const metrics = ctx.measureText(label);
  const mx = (m.x1 + m.x2) / 2, my = (m.y1 + m.y2) / 2;
  const angle = Math.atan2(dy, dx);
  const offX = -Math.sin(angle) * 20;
  const offY = Math.cos(angle) * 20;
...
function drawStrokeToCtx(ctx, s) {
  ...
  if (s.tool === 'pen') {
    ...
    // Live preview rendered as dashed line (same as measure overlay)
    if (!s.x2) { ctx.restore(); return; }
    const mDist = Math.sqrt((s.x2 - s.x1) ** 2 + (s.y2 - s.y1) ** 2);
    if (mDist < 1) { ctx.restore(); return; }
    ctx.strokeStyle = '#00ff88'; ctx.lineWidth = 2;
    ctx.setLineDash([6, 4]);
    ctx.beginPath(); ctx.moveTo(s.x1, s.y1); ctx.lineTo(s.x2, s.y2); ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = '#00ff88';
    [{x:s.x1,y:s.y1},{x:s.x2,y:s.y2}].forEach(p => { ctx.beginPath(); ctx.arc(p.x,p.y,4,0,Math.PI*2); ctx.fill(); });
    const mLabel = formatMeasurement(mDist);
    const mFontSize = 16;
    ctx.font = `600 ${mFontSize}px 'JetBrains Mono', monospace`;
    const mMetrics = ctx.measureText(mLabel);
    const mAngle = Math.atan2(s.y2-s.y1, s.x2-s.x1);
    const mmx = (s.x1+s.x2)/2 - Math.sin(mAngle)*20;
    const mmy = (s.y1+s.y2)/2 + Math.cos(mAngle)*20;
    ctx.save();
    ctx.translate(mmx, mmy);
    let mtAngle = mAngle;
    if (mtAngle > Math.PI/2) mtAngle -= Math.PI;
    if (mtAngle < -Math.PI/2) mtAngle += Math.PI;
    ctx.rotate(mtAngle);
    ctx.fillStyle = 'rgba(0,0,0,0.8)';
    ctx.fillRect(-mMetrics.width/2-4, -mFontSize/2-4, mMetrics.width+8, mFontSize+8);
    ctx.fillStyle = '#00ff88';
    ...
    if (dist < 1) { ctx.restore(); return; }
    const angle = Math.atan2(dy, dx);
    const tickLen = Math.max(8, s.width * 4);
    ...
    ctx.beginPath();
    ctx.moveTo(s.x1 - perpX, s.y1 - perpY);
    ctx.lineTo(s.x1 + perpX, s.y1 + perpY);
    ctx.moveTo(s.x2 - perpX, s.y2 - perpY);
    ctx.lineTo(s.x2 + perpX, s.y2 + perpY);
    ctx.stroke();
    // Label
    const label = formatMeasurement(dist);
    const fontSize = Math.max(12, s.width * 4);
    ctx.font = `600 ${fontSize}px 'JetBrains Mono', monospace`;
    const metrics = ctx.measureText(label);
    const mx = (s.x1 + s.x2) / 2, my = (s.y1 + s.y2) / 2;
    // Offset label perpendicular to the line
    const labelOffX = -Math.sin(angle) * (tickLen + fontSize * 0.5);
    const labelOffY = Math.cos(angle) * (tickLen + fontSize * 0.5);
    const lx = mx + labelOffX, ly = my + labelOffY;
    // Background pill behind text
    const pad = 3;
    ctx.save();
    ctx.translate(lx, ly);
    // Rotate text to align with line (keep readable)
    let textAngle = angle;
    if (textAngle > Math.PI / 2) textAngle -= Math.PI;
    if (textAngle < -Math.PI / 2) textAngle += Math.PI;
    ctx.rotate(textAngle);
...
function strokeHitsPoint(s, px, py, radius) {
  ...
  } else if (s.tool === 'line' || s.tool === 'arrow' || s.tool === 'dimension') {
...
function initDrawing() {
  ...
  [
    ...
    drawCanvas.addEventListener('mousedown', e => {
      ...
      if (drawTool === 'measure') {
        drawing = true;
        _measureResult = null;
        _drawCurrent = { tool: 'measure', x1: pos.x, y1: pos.y, x2: null, y2: null, color: '#00ff88', width: 2 };
        return;
      }
  ...
  window.addEventListener('mouseup', () => {
    ...
    if ((s.tool === 'line' || s.tool === 'arrow' || s.tool === 'rect' || s.tool === 'dimension') && s.x2 != null) valid = true;
    if (s.tool === 'eraser') valid = false;
    // Measure tool: show result overlay, don't save as stroke
    if (s.tool === 'measure' && s.x2 != null) {
      const distPx = pixelDistance(s.x1, s.y1, s.x2, s.y2);
      _measureResult = { x1: s.x1, y1: s.y1, x2: s.x2, y2: s.y2 };
      if (_calibratingMode) {
        _drawCurrent = null; drawSide = null;
        finishCalibration(distPx);
        renderAllDrawLayers();
        return;
      }
      valid = false;
